...
function isTriangle(a, b, c) {
  ...
  const otherSides = [a, b, c].filter((el) => el !== longestSide);
  return otherSides.length === 0 ? true : longestSide < otherSides.reduce((acc, el) => acc + el);
}
...
function doRectanglesOverlap(rect1, rect2) {
  const rect1Points = [];
  for (let i = 0; i < rect1.top + rect1.height; i += 1) {
    const str = new Array(rect1.width).fill(0);
    rect1Points.push(str);
  }
  ...
  let result = 0;
  for (let i = rect2.top; i <= rect2.top + rect2.height; i += 1) {
    for (let j = rect2.left; j <= rect2.left + rect2.width; j += 1) {
      if (rect1Points[i]) {
        if (rect1Points[i][j] === 'R') {
          result += 1;
        }
      }
    }
...
function findFirstSingleChar(str) {
  ...
  return uniqueChars[0] ? uniqueChars[0] : null;
}
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  const arr = [a, b].sort((min, max) => min - max);
...
function getDigitalRoot(num) {
  let result;
  const digits = num.toString().split('').map((el) => +el);
  const newNumber = digits.reduce((acc, el) => acc + el);
  if (newNumber > 9) {
    result = getDigitalRoot(newNumber);
  } else {
    result = newNumber;
  }
  return result;
