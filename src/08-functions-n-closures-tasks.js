...
function getPolynom(...args) {
  const k = args;
  const i = [];
  k.map((el, ind) => i.push(ind));
  i.reverse();
  return function checkPoint(x) {
    let y = 0;
...
function partialUsingArguments(fn, ...args) {
  return function continueSum(...remainingArgs) {
    const allArgs = args.concat(remainingArgs);
    return fn(...allArgs);
  };
}
